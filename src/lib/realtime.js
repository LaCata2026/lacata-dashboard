...
export const Realtime = {
  ...
  _manualClose: false,
  subscribe(table, fn) {
    if (!this.callbacks[table]) this.callbacks[table] = []
    this.callbacks[table].push(fn)
    return () => {
      this.callbacks[table] = this.callbacks[table].filter((f) => f !== fn)
    }
  },
  connect(token) {
    ...
    this.ws.onopen = () => {
      ...
      ;['tareas', 'usuarios', 'equipos'].forEach((table) => {
        this.ws.send(
          JSON.stringify({
      ...
      if (onClick)
        n.onclick = () => {
          ...
          try {
            ...
            console.warn('[PushNotif] onclick error:', e)
          }
          n.close()
        }
      setTimeout(() => {
