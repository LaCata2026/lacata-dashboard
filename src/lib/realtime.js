...
export const Realtime={
  ...
  connect(token){
    ...
    try{this.ws=new WebSocket(wsUrl)}catch(e){console.warn("Realtime WS:",e);return}
    this.ws.onopen=()=>{
    ...
    this.ws.onerror=()=>{}
    this.ws.onclose=()=>{clearInterval(this._heartbeat);if(!this._manualClose){this._reconnectTimer=setTimeout(()=>{this._reconnectDelay=Math.min(this._reconnectDelay*1.5,30000);this.connect(this._token)},this._reconnectDelay)}}
  },
  disconnect(){this._manualClose=true;clearInterval(this._heartbeat);clearTimeout(this._reconnectTimer);if(this.ws)try{this.ws.close()}catch{};this.ws=null},
}
