...
export function getInitials(name){return name.split(" ").map(w=>w[0]).join("").substring(0,2).toUpperCase()}
...
export const sb={
  async signIn(email,pw){
    ...
    const d=await r.json();if(d.error||d.error_description)throw new Error("Correo o contraseña incorrectos")
    const profileR=await fetch(`${SB_URL}/rest/v1/usuarios?id=eq.${d.user.id}&select=*`,{headers:{apikey:SB_ANON,Authorization:`Bearer ${d.access_token}`}})
    const profileData=await profileR.json();let profile
    if(Array.isArray(profileData)&&profileData.length>0){profile=profileData[0]}
    else{profile={id:d.user.id,email:d.user.email,name:d.user.user_metadata?.full_name||d.user.email.split("@")[0],role:getRole(d.user.email),avatar_color:getAvatarColor(d.user.email),initials:getInitials(d.user.user_metadata?.full_name||d.user.email.split("@")[0])}}
