...
export const statusPill = {
  ...
  en_progreso: 'pill-progreso',
  en_pausa: 'pill-pausa',
  en_revision: 'pill-revision',
...
export const statusColor = {
  ...
  completada: 'var(--s-completada)',
  vencida: 'var(--s-vencida)',
}
export const prioPill = {
  Normal: 'pill-prio-normal',
  Alta: 'pill-prio-alta',
  Urgente: 'pill-prio-urgente',
}
export const NAV = [
  {
    id: 'home',
    ...
    label: 'Inicio',
    section: 'trabajo',
    roles: ['director', 'colaborador', 'cuentas'],
  },
  {
    id: 'ordenes',
    icon: 'ordenes',
...
export const fmtDateRelative = (d, status) => {
  ...
  if (diff < 0)
    return { label: `Venció hace ${Math.abs(diff)}d`, color: 'var(--red)', urgent: true }
...
export function useSessionFilters(key, defaults) {
  const stored = (() => {
    try {
      const v = sessionStorage.getItem('sf_' + key)
  ...
  function set(val) {
    try {
      sessionStorage.setItem('sf_' + key, JSON.stringify(val))
    } catch {}
    setState(val)
  }
  return [state, set]
...
export async function syncVencidas(tasks, token, sb) {
  ...
  const toRevert = tasks.filter(
    (t) => t.status === 'vencida' && t.due_date && new Date(t.due_date + 'T00:00:00') >= today
  )
