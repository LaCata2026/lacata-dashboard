...
export function ReassignModal({ task, users, teams, token, onClose, onRefresh, me }) {
  const current = Array.isArray(task.assigned_to)
    ? task.assigned_to
  ...
  async function save() {
    if (selected.length === 0) {
      showToast('Debe haber al menos un responsable', 'error')
      return
    }
    setSaving(true)
    const h = [
      ...(task.history || []),
      `Responsables actualizados por ${me.name} — ${new Date().toLocaleString('es-GT')}`,
    ]
          ...
          {opts.map((u) => {
            ...
            return (
              <div
                key={u.id}
                onClick={() =>
                  setSelected((s) => (sel ? s.filter((x) => x !== u.id) : [...s, u.id]))
                }
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  borderRadius: 8,
                  cursor: 'pointer',
                  background: sel ? 'var(--bg4)' : 'transparent',
...
export function AddChangeModal({ task, token, onClose, onRefresh, me }) {
  ...
  async function save() {
    ...
    const h = [
      ...(task.history || []),
      `Cambio ${n}: ${text.trim()} — ${me.name} — ${new Date().toLocaleString('es-GT')}`,
    ]
  ...
  return ReactDOM.createPortal(
    <div className="confirm-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="confirm-box fade-in">
        ...
        >
          Registrar cambio #{(task.changes || 0) + 1}
        </h3>
        <p
          style={{
            fontSize: 12,
  ...
  try {
    ...
    await sb.insert(
      'tareas',
      {
        ...
        comments: [],
        history: [
          `Orden AC-${String(orderNum).padStart(4, '0')} creada (copia de AC-${String(task.order_number || 0).padStart(4, '0')}) — ${new Date().toLocaleDateString('es-GT')}`,
        ],
      },
      token
    )
  ...
  return (
    <div style={{ display: 'flex', gap: 6 }}>
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' && txt.trim()) onSubmit(txt.trim())
        }}
      ...
      <button
        className="btn btn-primary btn-sm"
        disabled={!txt.trim()}
        onClick={() => txt.trim() && onSubmit(txt.trim())}
      >
        OK
      </button>
    </div>
...
export default function TaskCard({
  ...
  useEffect(() => {
    if (activeTab === 'conversacion' && commentsEndRef.current)
  ...
  async function changeStatus(s, pauseReason) {
    ...
    const now = new Date()
    const nowStr = now.toLocaleString('es-GT')
    const prev = task.status
    let newHoursReal = Number(task.hours_real) || 0
    let entry = ''
    if (
      prev === 'en_progreso' &&
      task.started_at &&
      (s === 'en_pausa' || s === 'en_revision' || s === 'completada')
    ) {
      const started = new Date(task.started_at)
      const diffH = Math.round(((now - started) / 3600000) * 100) / 100
      newHoursReal = Math.round((newHoursReal + diffH) * 100) / 100
      if (s === 'en_revision')
        entry = `🔍 ${me.name} envió a revisión — ${nowStr} (⏱ ${diffH}h sesión · total: ${newHoursReal}h — reloj detenido)`
      else if (s === 'en_pausa')
        entry = `⏸ ${me.name} → Pausa: ${pauseReason} — ${nowStr} (${diffH}h sesión · total: ${newHoursReal}h)`
      else entry = `✅ ${me.name} completó — ${nowStr} — Total: ${newHoursReal}h`
    } else if (s === 'en_progreso') {
      entry = `⚡ ${me.name} inició trabajo — ${nowStr}`
    } else if (s === 'en_revision' && prev !== 'en_progreso') {
      entry = `🔍 ${me.name} envió a revisión — ${nowStr}`
    } else if (s === 'completada') {
      entry = `✅ ${me.name} marcó como completada — ${nowStr} — Total: ${newHoursReal}h reales`
    } else if (s === 'en_pausa') {
      entry = `⏸ ${me.name} → Pausa: ${pauseReason} — ${nowStr}`
    } else {
      entry = `${statusLabel[s]} — ${me.name} — ${nowStr}`
    }
    const h = [...(task.history || []), entry]
    const updates = {
      status: s,
      history: h,
      hours_real: newHoursReal,
      started_at:
        s === 'en_progreso'
          ? now.toISOString()
          : prev === 'en_progreso'
  ...
  async function del() {
    ...
    for (const f of files) {
      if (typeof f === 'object' && f.path) {
        try {
          await sb.deleteFile(f.path)
        } catch (e) {
          console.warn('No se pudo borrar archivo:', f.path)
        }
      }
    }
  ...
  async function saveEdit() {
    const h = [
      ...(task.history || []),
      `Editado por ${me.name} — ${new Date().toLocaleString('es-GT')}`,
    ]
    await sb.update(
      'tareas',
      task.id,
      {
        ...
        description: editForm.description || task.description,
        materials: editForm.materials || task.materials || '',
        hours: Number(editForm.hours) || task.hours || 0,
        due_date: editForm.due_date || task.due_date,
  ...
  async function postComment() {
    ...
    try {
      ...
      users.forEach((u) => {
        ...
        if (trimmed.toLowerCase().includes(handle)) mentions.push(u.id)
      })
      const newComment = {
        id: Date.now() + '-' + Math.random().toString(36).slice(2, 8),
        user_id: me.id,
        user_name: me.name,
        user_color: me.avatar_color || '#888',
        text: trimmed,
        mentions,
      ...
      showToast('Error al enviar comentario: ' + e.message, 'error')
    }
  ...
  function insertMention(user) {
    ...
    const before = commentText.substring(0, mentionState.pos)
    const after = commentText.substring(mentionState.pos)
    const newBefore = before.replace(/@\w*$/, `@${firstName} `)
  ...
  const existingMarcas = [
    ...new Set([...marcasDB, ...(allTasks || []).map((t) => t.marca).filter(Boolean)]),
  ].sort()
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {task.order_number && (
              ...
              >
                #{'AC-' + String(task.order_number).padStart(4, '0')}
              </span>
            )}
            <span className={`pill ${prioPill[task.priority] || 'pill-gray'}`}>
              {task.priority}
            </span>
            {(() => {
              const canChange =
                isDir ||
                (Array.isArray(task.assigned_to) ? task.assigned_to : [task.assigned_to]).includes(
                  me.id
                )
              if (!canChange)
                return (
                  <span className={`pill ${statusPill[effStatus] || 'pill-gray'}`}>
                    {statusLabel[effStatus] || task.status}
                  </span>
                )
              const opts = [
              ...
              return (
                <span style={{ display: 'inline-flex' }}>
                  <span
                    className={`pill ${statusPill[effStatus] || 'pill-gray'}`}
                    onClick={(e) => {
                      e.stopPropagation()
                      if (quickMenu) {
                        setQuickMenu(false)
                          ...
                          {opts.map((o) => (
                            <span
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                setQuickMenu(false)
                                if (o.v !== effStatus) changeStatus(o.v)
                              }}
                              style={{
                                padding: '7px 12px',
                                borderRadius: 5,
                                fontSize: 12,
                                cursor: 'pointer',
                                color: o.v === effStatus ? 'var(--accent)' : 'var(--text)',
                                fontWeight: o.v === effStatus ? 700 : 400,
                                background: o.v === effStatus ? 'var(--accent-dim)' : 'transparent',
                                fontFamily: 'var(--font-body)',
                                whiteSpace: 'nowrap',
                                transition: '.1s',
                              }}
            ...
            {sinMarca && isDir && (
              ...
              >
                <Icon n="alerta" size={9} /> Sin marca
              </span>
            )}
            {task.changes > 0 && (
              <span className="pill pill-purple">
                <Icon n="cambio" size={10} /> {task.changes}
              </span>
            )}
            {(() => {
              const files = Array.isArray(task.files) ? task.files : []
              ...
              const now = Date.now()
              const urgent = files.some(
                (f) =>
                  typeof f === 'object' &&
                  f.uploaded_at &&
                  new Date(f.uploaded_at).getTime() + 48 * 3600000 - now < 6 * 3600000 &&
                  new Date(f.uploaded_at).getTime() + 48 * 3600000 - now > 0
              )
              const warning =
                !urgent &&
                files.some(
                  (f) =>
                    typeof f === 'object' &&
                    f.uploaded_at &&
                    new Date(f.uploaded_at).getTime() + 48 * 3600000 - now < 24 * 3600000 &&
                    new Date(f.uploaded_at).getTime() + 48 * 3600000 - now > 0
                )
              if (urgent)
                return (
                  ...
                  >
                    <Icon n="alerta" size={10} /> Archivo expira pronto
                  </span>
                )
              if (warning)
                return (
                  <span
                    className="pill"
                    style={{ background: 'rgba(232,140,46,.12)', color: 'var(--p-alta)' }}
                  >
                    <Icon n="alerta" size={10} /> Archivo vence hoy
                  </span>
                )
              return (
                <span className="pill" style={{ background: 'var(--bg4)', color: 'var(--muted2)' }}>
                  <Icon n="adjunto" size={10} /> {files.length}
                </span>
              )
            })()}
            {comments.length > 0 && (
              <span
                className="pill"
                style={{ background: 'rgba(155,127,232,.12)', color: 'var(--s-revision)' }}
              >
                <Icon n="comentar" size={10} /> {comments.length}
              </span>
            )}
            {team &&
              (() => {
                ...
                return (
                  <span className="pill" style={{ background: tc + '22', color: tc }}>
                    {team.name}
                  </span>
                )
              })()}
          ...
          >
            {(() => {
              const al = Array.isArray(task.assigned_to)
                ? task.assigned_to
                : [task.assigned_to].filter(Boolean)
              return al
            ...
            {(() => {
              const dr = fmtDateRelative(task.due_date, effStatus)
              return (
                <span
                  style={{
                    color: dr.color,
                    fontWeight: dr.urgent ? 700 : 400,
                    background: dr.urgent ? 'rgba(240,107,107,.08)' : 'transparent',
                    padding: dr.urgent ? '2px 6px' : '0',
                    borderRadius: 5,
                  }}
                >
                  <Icon n="tiempo" size={11} style={{ marginRight: 3 }} />
                  {dr.label}
                </span>
              )
            })()}
            ...
            {task.hours_real > 0 && localStatus !== 'en_progreso' && (
              <span style={{ color: 'var(--s-completada)' }}>✓ {task.hours_real}h real</span>
            )}
            {lastActivity && (
              <span style={{ color: 'var(--muted)', fontSize: 11 }}>{lastActivity}</span>
            )}
          </div>
        </div>
        <span className="task-open-hint">
          Ver detalle <Icon n="flecha_der" size={11} />
        </span>
      </div>
            ...
            <div className="modal fade-in" onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ flex: 1 }}>
                  {task.order_number && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      >
                        {'AC-' + String(task.order_number).padStart(4, '0') + ' ' + task.title}
                      </span>
                      <button
                        onClick={(e) => {
              ...
              {sinMarca && isDir && (
                ...
                >
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--yellow)', flex: 1 }}>
                    Esta orden no tiene marca asignada
                  </span>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    {marcaInput === '__nueva__' && (
                      <input
                        ...
                        value={marcaInput === '__nueva__' ? '' : marcaInput}
                        onChange={(e) => setMarcaInput(e.target.value)}
                        style={{
                          fontSize: 12,
                          padding: '5px 10px',
                    ...
                    <button
                      onClick={saveMarca}
                      disabled={!marcaInput || marcaInput === '__nueva__' || savingMarca}
                      style={{
                        fontSize: 12,
                        fontWeight: 700,
                        padding: '5px 14px',
                        ...
                        color: '#0d0d0d',
                        cursor:
                          !marcaInput || marcaInput === '__nueva__' ? 'not-allowed' : 'pointer',
                        opacity: !marcaInput || marcaInput === '__nueva__' ? 0.5 : 1,
                        fontFamily: 'var(--font-body)',
                      }}
                    >
                      {savingMarca ? '...' : 'Asignar'}
              ...
              >
                ...
                <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '10px 14px' }}>
                  ...
                  <p style={{ fontSize: 13 }}>
                    <Icon n="tiempo" size={12} style={{ marginRight: 4 }} />
                    {(() => {
                      const dr = fmtDateRelative(task.due_date, effStatus)
                      return (
                        <span style={{ color: dr.color, fontWeight: dr.urgent ? 700 : 400 }}>
                          {dr.label}
                        </span>
                      )
                    })()}
                  ...
                  <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                    ⏱ Est: {task.hours || 0}h{' '}
                    {task.hours_real > 0 && (
                      <span style={{ color: 'var(--green)' }}>· Real: {task.hours_real}h</span>
                    )}
                  </p>
                </div>
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        border: '1px solid rgba(232,140,46,.25)',
                      }}
                    >
                      <Icon n="alerta" size={12} color="var(--p-alta)" />
                      <span style={{ fontSize: 11, color: 'var(--p-alta)', fontWeight: 600 }}>
                        Los archivos se eliminan a las 48h de subidos.
                      </span>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    {task.files.map((f, idx) => {
                      ...
                      const uploadedAt =
                        typeof f === 'object' && f.uploaded_at ? new Date(f.uploaded_at) : null
                      const fileExt = (fileName || '').split('.').pop().toLowerCase()
                      const isImg = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'].includes(fileExt)
                      const expiryInfo = (() => {
                        if (!uploadedAt) return null
                        const expiry = new Date(uploadedAt.getTime() + 48 * 3600000)
                        const remaining = expiry - Date.now()
                        if (remaining <= 0)
                          return { label: 'Expirado', color: 'var(--s-vencida)', urgent: true }
                        const h = Math.floor(remaining / 3600000)
                        const m = Math.floor((remaining % 3600000) / 60000)
                        if (h < 6)
                          return {
                            label: `${h}h ${m}m restantes`,
                            color: 'var(--s-vencida)',
                            urgent: true,
                          }
                        if (h < 24)
                          return { label: `${h}h restantes`, color: 'var(--p-alta)', urgent: false }
                          ...
                          >
                            <div style={{ position: 'relative' }}>
                              ...
                              {expiryInfo?.urgent && (
                                ...
                                >
                                  {expiryInfo.label.replace(' restantes', '')}
                                </div>
                              )}
                            </div>
                            <span
                        ...
                        <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                          <a
                            ...
                            style={{
                              ...
                              borderRadius: 8,
                              color: 'var(--accent)',
                              border: `1px solid ${expiryInfo?.urgent ? 'rgba(232,93,93,.3)' : 'rgba(232,197,71,.2)'}`,
                              display: 'flex',
                              alignItems: 'center',
                              gap: 6,
                              textDecoration: 'none',
              ...
              >
                {[
                  { v: 'detalles', icon: 'detalles', c: (task.history || []).length },
                  { v: 'conversacion', icon: 'comentar', c: comments.length },
                ].map((t) => (
                  <button
                    key={t.v}
                    ...
                    style={{
                      ...
                      border: 'none',
                      borderBottom:
                        activeTab === t.v ? '2px solid var(--accent)' : '2px solid transparent',
                      padding: '8px 14px',
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                      fontSize: 13,
                      color: activeTab === t.v ? 'var(--text)' : 'var(--muted)',
                      fontWeight: activeTab === t.v ? 700 : 500,
                      marginBottom: -1,
                      transition: '.13s',
                    }}
                  >
                    <Icon n={t.icon} size={13} style={{ marginRight: 5 }} />
                    {t.v === 'detalles' ? 'Detalles' : 'Conversación'}
                    {t.c > 0 && (
                      <span
                        style={{
                          marginLeft: 6,
                          ...
                          padding: '1px 6px',
                          borderRadius: 10,
                          background: activeTab === t.v ? 'var(--accent)' : 'var(--bg4)',
                          color: activeTab === t.v ? '#0d0d0d' : 'var(--muted2)',
                          fontWeight: 700,
                          fontFamily: 'var(--font-mono)',
                        }}
                      >
              ...
              {activeTab === 'detalles' && (
                <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '12px 14px' }}>
                  ...
                  {(task.history || []).length === 0 ? (
                    <p style={{ fontSize: 13, color: 'var(--muted)' }}>Sin historial aun.</p>
                  ) : (
                ...
                <div>
                  ...
                  >
                    {comments.length === 0 ? (
                      ...
                      >
                        <div style={{ fontSize: 32, opacity: 0.4, marginBottom: 6 }}>
                          <Icon n="comentar" size={32} color="currentColor" />
                        </div>
                      ...
                      comments.map((c) => {
                        ...
                        const timeStr = t.toLocaleString('es-GT', {
                          day: '2-digit',
                          month: 'short',
                          hour: '2-digit',
                          minute: '2-digit',
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              paddingLeft: mentioned ? 8 : 0,
                              marginLeft: mentioned ? -8 : 0,
                            }}
                          >
                            <Av
                              u={
                                ...
                                {(() => {
                                  ...
                                  return parts.map((p, i) => {
                                    if (p.startsWith('@')) {
                                      ...
                                      const u = users.find(
                                        (x) => x.name.split(' ')[0].toLowerCase() === name
                                      )
                                      return u ? (
                                        <span
                                          key={i}
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {mentionState.open &&
                      (() => {
                        const matches = users
                          .filter((u) => u.name.toLowerCase().includes(mentionState.query))
                          .slice(0, 5)
                  ...
                  >
                    ⌘+Enter para enviar · @nombre para mencionar
                  </p>
                </div>
              )}
              ...
              {isDir && (
                ...
                >
                  <button className="btn btn-ghost btn-sm" onClick={() => setShowReassign(true)}>
                    <Icon n="equipo2" size={14} /> Reasignar
                  </button>
                  <button className="btn btn-ghost btn-sm" onClick={() => setShowAddChange(true)}>
                    <Icon n="cambio" size={14} /> + Cambio
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() =>
                      duplicateTask(task, token, () => {
                        onRefresh()
                      })
                    }
                  >
                    <Icon n="duplicar" size={14} /> Duplicar
                  </button>
                  <button className="btn btn-danger btn-sm" onClick={del}>
                    <Icon n="eliminar" size={14} /> Eliminar
                  </button>
                </div>
              )}
            </div>
      ...
      {pausePrompt &&
        ReactDOM.createPortal(
          <div
            ...
            onClick={(e) => e.target === e.currentTarget && setPausePrompt(false)}
          >
            <div className="confirm-box fade-in" style={{ maxWidth: 420 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
                {[
                  'Cambió la prioridad',
                  'Continúa otro día',
                  'Esperando algo (material/feedback)',
                ].map((motivo) => (
                  <button
                    key={motivo}
                    onClick={() => {
                      setPausePrompt(false)
                      changeStatus('en_pausa', motivo)
                    }}
                    style={{
                      textAlign: 'left',
                      padding: '11px 14px',
                ...
                <OtherReasonInput
                  onSubmit={(txt) => {
                    setPausePrompt(false)
                    changeStatus('en_pausa', txt)
                  }}
                />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button className="btn btn-ghost btn-sm" onClick={() => setPausePrompt(false)}>
                  Cancelar
                </button>
              </div>
            </div>
