...
function getRanges() {
  ...
  const semFrom = new Date(now)
  semFrom.setDate(diff)
  semFrom.setHours(0, 0, 0, 0)
  const semTo = new Date(semFrom)
  semTo.setDate(semTo.getDate() + 6)
  ...
  return tasks.filter((t) => {
    ...
    if (period === 'custom' && customFrom && customTo) {
      const f = new Date(customFrom + 'T00:00:00'),
        to2 = new Date(customTo + 'T23:59:59')
...
async function buildExcel(tasks, users, teams, period, customFrom, customTo) {
  ...
  filtered.forEach((t) => {
    ...
    if (!marcasMap[key])
      marcasMap[key] = { marca: key, tasks: [], hrsE: 0, hrsR: 0, colabs: new Set(), hasMarca: !!m }
  ...
  wsResumen.columns = [
    ...
    { header: 'Colaboradores', width: 14 },
    { header: 'Hrs Estimadas', width: 14 },
    { header: 'Hrs Reales', width: 12 },
    { header: 'Eficiencia', width: 12 },
    { header: 'Completadas', width: 14 },
    { header: 'Vencidas', width: 10 },
  ]

  wsResumen.insertRow(1, [`REPORTE LA CATA — ${periodStr.toUpperCase()}`])
  wsResumen.insertRow(2, [])
  wsResumen.getRow(1).font = { bold: true, size: 13 }
  wsResumen.getRow(3).font = { bold: true }
  wsResumen.getRow(3).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1a1a2e' } }
  wsResumen.getRow(3).font = { bold: true, color: { argb: 'FFFFFFFF' } }
  ...
  if (sinMarca && sinMarca.tasks.length > 0) {
    wsResumen.addRow([])
    wsResumen.addRow([`⚠️ ${sinMarca.tasks.length} orden(es) sin marca asignada`])
  }
  ...
  const colWidths = [10, 32, 24, 16, 14, 10, 10, 10, 11, 14, 14, 9]

  marcasConNombre.forEach(({ marca, tasks: mt }) => {
    const sheetName = marca.replace(/[:\\/?*[\]]/g, '').slice(0, 31)
    const ws = wb.addWorksheet(sheetName)
    ws.columns = header.map((h, i) => ({ header: h, width: colWidths[i] }))
  ...
  wsColabs.columns = [
    ...
    { header: 'Marca(s)', width: 24 },
    { header: 'Total Órdenes', width: 14 },
    { header: 'Completadas', width: 13 },
    { header: 'Vencidas', width: 10 },
    { header: 'Hrs Est.', width: 10 },
    { header: 'Hrs Reales', width: 11 },
    { header: 'Eficiencia', width: 11 },
  ]
  wsColabs.getRow(1).font = { bold: true }
  wsColabs.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1a1a2e' } }
  wsColabs.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } }

  Object.values(colabMap)
    .sort((a, b) => {
      const ua = users.find((x) => x.id === a.id)?.name || ''
      const ub = users.find((x) => x.id === b.id)?.name || ''
    ...
    .forEach((c) => {
      ...
      wsColabs.addRow([
        ...
        [...c.marcas].join(', ') || '—',
        c.tasks.length,
        c.tasks.filter((t) => t.status === 'completada').length,
        c.tasks.filter((t) => t.status === 'vencida').length,
        fmtH(hrsE),
        fmtH(hrsR),
        eff(hrsE, hrsR),
      ])
  ...
  const filename = `LaCata_Reporte_${periodLabel}_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  showToast(
    `Reporte descargado${sinMarca?.tasks.length ? ` · ${sinMarca.tasks.length} órdenes sin marca` : ''}`,
    sinMarca?.tasks.length ? 'warning' : 'success'
  )
}
...
export default function ReporteExcel({ tasks, users, teams, isOpen, onClose }) {
  ...
  async function handleDownload() {
    ...
    setLoading(true)
    try {
      await buildExcel(tasks, users, teams, period, customFrom, customTo)
    } catch (e) {
      console.error(e)
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20 }}>
          Excel con pestañas por marca, órdenes, colaboradores y horas
        </p>
        ...
        <div
          style={{
            ...
            borderRadius: 8,
            padding: '10px 12px',
            marginBottom: sinMarcaCount > 0 ? 12 : 20,
            fontSize: 11,
            color: 'var(--muted)',
            lineHeight: 1.8,
          }}
        >
          Pestañas incluidas:
          <br />
          <strong style={{ color: 'var(--text)' }}>Resumen</strong> → totales por marca
          <br />
          <strong style={{ color: 'var(--text)' }}>Una pestaña por marca</strong> → órdenes
          detalladas
          <br />
        ...
        {sinMarcaCount > 0 && (
          <div
            style={{
              ...
              border: '1px solid rgba(232,140,46,.3)',
              borderRadius: 8,
              padding: '8px 12px',
              marginBottom: 16,
              fontSize: 11,
          ...
          >
            ...
            <span>
              <strong>
                {sinMarcaCount} orden{sinMarcaCount !== 1 ? 'es' : ''} sin marca
              </strong>{' '}
              — no aparecerán en pestañas individuales.
            </span>
          </div>
        )}
