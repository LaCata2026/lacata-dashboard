...
export default function AdminView({
  ...
  async function createUser() {
    ...
    setLoading(true)
    try {
      const authUser = await sb.inviteUser(uForm.email, uForm.name, token)
      ...
      const teamId =
        uForm.role === 'cuentas'
          ? (uForm.team_ids && uForm.team_ids[0]) || null
          : uForm.team_id || null
  ...
  async function createTeam() {
    ...
    try {
      await sb.insert(
        'equipos',
        { name: tForm.name, color: tForm.color, icon: tForm.icon || 'equipos' },
        token
      )
  ...
  async function saveTeamEdit() {
    ...
    await sb.update(
      'equipos',
      editingTeam.id,
      { name: editingTeam.name, color: editingTeam.color, icon: editingTeam.icon || 'equipos' },
      token
    )
  ...
  async function deleteUser(u) {
    const ok = await showConfirm(`¿Desactivar a ${u.name}?`, {
  ...
  async function toggleUserTeam(userId, teamId, currentTeamIds, role) {
    ...
    setUsers((prev) =>
      prev.map((u) => (u.id === userId ? { ...u, team_ids: next, team_id: primaryId } : u))
    )
    try {
      ...
      showToast(
        next.length === 0
          ...
          : `${next.length} equipo${next.length > 1 ? 's' : ''} asignado${next.length > 1 ? 's' : ''}`,
        'success'
      )
      onRefresh()
    } catch (e) {
      setUsers((prev) =>
        prev.map((u) => (u.id === userId ? { ...u, team_ids: ids, team_id: ids[0] || null } : u))
      )
  ...
  return (
    <div>
      ...
      <div className="card fade-in" style={{ marginBottom: 12 }}>
        <div
          ...
          style={{
            ...
            cursor: 'pointer',
            userSelect: 'none',
            marginBottom: secOpen.equipos ? 16 : 0,
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span
              style={{
                ...
                transform: secOpen.equipos ? 'rotate(0)' : 'rotate(-90deg)',
              }}
            >
              ▼
            </span>
          ...
          <>
            ...
            {teams.map((t) => {
              ...
              const activeCount = tasks.filter(
                (x) => x.team_id === t.id && x.status !== 'completada'
              ).length
              ...
              const overdueCount = tasks.filter(
                (x) => x.team_id === t.id && x.status === 'vencida'
              ).length
              const sColor =
              ...
              return (
                ...
                >
                  ...
                  />
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 600, minWidth: 80 }}>
                    {t.name}
                  </span>
                  ...
                  <button
                    ...
                    onClick={() => setEditingTeam({ ...t, icon: t.icon || 'equipos' })}
                  >
                    Editar
                  </button>
                  <button className="btn btn-danger btn-sm" onClick={() => deleteTeam(t)}>
                    Eliminar
                  </button>
                </div>
              )
      ...
      {inactiveUsers.length > 0 && (
        <div className="card fade-in" style={{ marginTop: 12, opacity: 0.85 }}>
          <div
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span
                style={{
                  ...
                  transition: 'transform .2s',
                  display: 'inline-block',
                  transform: showInactive ? 'rotate(0)' : 'rotate(-90deg)',
                }}
              >
                ▼
              </span>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--muted)' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>
              Su historial de órdenes está preservado
            </span>
          </div>
          {showInactive && (
              ...
              {inactiveUsers.map((u) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--muted2)' }}>
                      {u.name}
                    </p>
          ...
          >
            <div className="modal fade-in" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="form-group">
                ...
                <input value={uForm.name} onChange={setU('name')} placeholder="Ej: Lucia Mendoza" />
              </div>
              <div className="form-group">
              ...
              {uForm.role === 'cuentas' ? (
                <div className="form-group">
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            cursor: 'pointer',
                            fontFamily: 'inherit',
                            background: sel ? teamColor(t) : 'var(--bg4)',
                            color: sel ? '#fff' : 'var(--muted2)',
                  ...
                  {(uForm.team_ids || []).length === 0 && (
                    <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 6 }}>
                      Sin restricción — verá todos los equipos
                    </p>
                  )}
                </div>
              ) : (
              ...
              {uForm.name && (
                ...
                >
                  <Av
                    u={{ initials: getInitials(uForm.name), avatar_color: uForm.avatar_color }}
                    size={36}
                  />
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 600 }}>{uForm.name}</p>
                    <p style={{ fontSize: 12, color: 'var(--muted)' }}>
                      {uForm.role}
                      {uForm.role === 'cuentas' && (uForm.team_ids || []).length > 0
                        ? ` · ${(uForm.team_ids || []).length} equipos`
                        : ''}
                    </p>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button className="btn btn-ghost" onClick={() => setShowNewUser(false)}>
                  Cancelar
                </button>
      ...
      {showNewTeam && (
        <ModalPortal>
          <div
            className="modal-overlay"
            onClick={(e) => e.target === e.currentTarget && setShowNewTeam(false)}
          >
            <div className="modal fade-in" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="form-group">
                ...
                <input
                  value={tForm.name}
                  onChange={setT('name')}
                  placeholder="Ej: Celula Estrategia"
                />
              </div>
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {TEAM_ICONS.map((ic) => (
                    <button
                      key={ic}
                      type="button"
                      onClick={() => setTForm((f) => ({ ...f, icon: ic }))}
                      style={{
                        fontSize: 20,
                        padding: '6px 10px',
                        borderRadius: 8,
                        background: tForm.icon === ic ? 'var(--accent)' : 'var(--bg3)',
                        border: tForm.icon === ic ? 'none' : '1px solid var(--border)',
                        cursor: 'pointer',
                      }}
                    >
                      {ic}
                  ...
                  {COLORS.map((c) => (
                    <div
                      ...
                      style={{
                        ...
                        background: c,
                        cursor: 'pointer',
                        border: tForm.color === c ? '2px solid #fff' : '2px solid transparent',
                        transform: tForm.color === c ? 'scale(1.15)' : 'scale(1)',
                        transition: '.15s',
                      }}
                    />
                  ))}
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button className="btn btn-ghost" onClick={() => setShowNewTeam(false)}>
                  Cancelar
                </button>
      ...
      {editingTeam && (
        <ModalPortal>
          <div
            className="modal-overlay"
            onClick={(e) => e.target === e.currentTarget && setEditingTeam(null)}
          >
            <div className="modal fade-in" onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {TEAM_ICONS.map((ic) => (
                    <button
                      key={ic}
                      type="button"
                      onClick={() => setEditingTeam((t) => ({ ...t, icon: ic }))}
                      style={{
                        fontSize: 20,
                        padding: '6px 10px',
                        borderRadius: 8,
                        background: editingTeam.icon === ic ? 'var(--accent)' : 'var(--bg3)',
                        border: editingTeam.icon === ic ? 'none' : '1px solid var(--border)',
                        cursor: 'pointer',
                      }}
                    >
                      {ic}
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
                  {COLORS.map((c) => (
                    <div
                      key={c}
                      onClick={() => setEditingTeam((t) => ({ ...t, color: c }))}
                      style={{
                        width: 28,
                        height: 28,
                        borderRadius: '50%',
                        ...
                        cursor: 'pointer',
                        border:
                          editingTeam.color === c ? '2px solid #fff' : '2px solid transparent',
                        transform: editingTeam.color === c ? 'scale(1.15)' : 'scale(1)',
                        transition: '.15s',
                      }}
                    />
                  ))}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
                  ...
                  >
                    <Icon n={editingTeam.icon || 'equipos'} size={18} />
                  </span>
                  <div
                    style={{
                      width: 10,
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button className="btn btn-primary" onClick={saveTeamEdit}>
                  Guardar cambios
                </button>
              </div>
            </div>
