  ...
  return (
    <button
      ...
      style={{
        ...
        alignItems: isHero ? 'baseline' : 'center',
        gap: isHero ? 8 : 3,
      }}
      onMouseEnter={(e) => {
        if (clickable) {
    ...
    >
      <span
        style={{
          fontSize: isHero ? 34 : 22,
          fontWeight: 800,
          color,
          fontFamily: 'var(--font-display)',
          letterSpacing: isHero ? '-.03em' : 'normal',
          lineHeight: 1,
        }}
      >
        {val}
      ...
      <span
        style={{
          ...
          letterSpacing: isHero ? '.1em' : '.05em',
          fontFamily: 'var(--font-mono)',
          marginTop: isHero ? 0 : 3,
        }}
      >
        {label}
      </span>
  ...
  switch (browser) {
    ...
    case 'opera':
      return {
        ...
        shortcut:
          browser === 'chrome'
            ? 'O ve a chrome://settings/content/notifications'
            : browser === 'edge'
              ? 'O ve a edge://settings/content/notifications'
              : browser === 'brave'
                ? 'O ve a brave://settings/content/notifications'
                : null,
      }
    case 'safari':
      return {
    ...
    case 'chrome-ios':
      return {
        steps: [
          'Chrome en iOS no soporta notificaciones de escritorio. Considera usar Safari o instalar la app como PWA.',
        ],
        shortcut: null,
      }
    default:
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ flex: 1, color: 'var(--muted2)', lineHeight: 1.5, minWidth: 200 }}>
          ...
          {isDenied && (
            <>
              <strong style={{ color: '#fca5a5' }}>
                Notificaciones bloqueadas en {browserName}.
              </strong>{' '}
              <button
          ...
          {!isGranted && !isDenied && !unsupported && (
            <>
              <strong style={{ color: 'var(--text)' }}>Activa las notificaciones</strong> para
              recibir avisos cuando te asignen una orden o te mencionen.
            </>
          )}
          {unsupported && (
            <>
        ...
        {justEnabled && isGranted && (
          <span
            className="fade-in"
            style={{ fontSize: 11, color: 'var(--s-completada)', fontWeight: 600, flexShrink: 0 }}
          >
            ✓ ¡Listo!
          </span>
        )}
      ...
      {isDenied && showHelp && (
        <div
          ...
          style={{
            marginTop: 10,
            paddingTop: 10,
            borderTop: '1px solid rgba(232,93,93,.2)',
...
function MyWeekCard({ me, tasks, onNavigate }) {
  ...
  const Metric = ({ val, label, color, tip }) => (
    ...
    >
      ...
      >
        {label}
        {tip && <span style={{ marginLeft: 4, opacity: 0.5 }}>ⓘ</span>}
      </div>
    </div>
  )
  const OpCount = ({ val, label, color, onClick }) => (
    <button
      ...
      onMouseEnter={(e) => {
        if (onClick) e.currentTarget.style.background = 'var(--bg3)'
      }}
      onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
    >
      <span style={{ fontSize: 16, fontWeight: 800, color, fontFamily: 'var(--font-display)' }}>
        {val}
      </span>
      <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 6 }}>{label}</span>
      {onClick && (
        <span style={{ fontSize: 10, color: 'var(--muted)', opacity: 0.4, marginLeft: 4 }}>→</span>
      )}
    </button>
  )
  return (
    ...
    >
      ...
      >
        ...
        >
          {weekStart.toLocaleDateString('es-GT', { day: '2-digit', month: 'short' })} –{' '}
          {weekEnd.toLocaleDateString('es-GT', { day: '2-digit', month: 'short' })}
        </span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
        <Metric
          val={doneThisWeek.length}
          label="Completadas"
          color="var(--s-completada)"
          tip="Órdenes que terminaste esta semana (lunes a domingo)"
        />
        <Metric
          val={efic == null ? '—' : efic + '%'}
          label="Eficiencia"
          ...
          tip="Horas estimadas vs. horas reales. 100% = trabajaste justo lo previsto"
        />
        <Metric
          val={`${streak}${streak >= 7 ? ' 🔥' : ''}`}
          label="Días sin atraso"
          color={streak >= 7 ? 'var(--accent)' : 'var(--text)'}
          tip="Días seguidos sin que se te venza ninguna tarea"
        />
      </div>
      <div
        style={{
      ...
      >
        <OpCount
          val={activas}
          label={activas === 1 ? 'activa' : 'activas'}
          color="var(--blue)"
        ...
        <OpCount
          ...
          onClick={() => onNavigate && onNavigate('ordenes', 'en_revision')}
        />
        <div style={{ width: 1, height: 24, background: 'var(--border)' }} />
        <OpCount
          val={vencidas}
          label={vencidas === 1 ? 'vencida' : 'vencidas'}
          color={vencidas > 0 ? 'var(--s-vencida)' : 'var(--s-completada)'}
          onClick={() => onNavigate && onNavigate('ordenes', 'vencida')}
        />
      </div>
    </div>
  )
...
export default function HomeView({
  ...
  const now = Date.now(),
    day7 = 7 * 24 * 3600000
  const completedThisWeek = scopedTasks.filter(
    (t) =>
      t.status === 'completada' && t.updated_at && now - new Date(t.updated_at).getTime() < day7
  ).length
  const completedLastWeek = scopedTasks.filter(
    (t) =>
      t.status === 'completada' &&
      t.updated_at &&
      now - new Date(t.updated_at).getTime() < day7 * 2 &&
      now - new Date(t.updated_at).getTime() >= day7
  ).length
  const trend =
    completedLastWeek === 0
      ? null
  ...
  const collabs = users.filter((u) => {
    ...
    return (
      ...
      (Array.isArray(u.team_ids) && u.team_ids.some((id) => myTeamIds.includes(id)))
    )
  })
  const workload = collabs
    .map((u) => ({
      ...u,
      active: scopedTasks.filter((t) => {
        const a = Array.isArray(t.assigned_to) ? t.assigned_to : [t.assigned_to].filter(Boolean)
        ...
        <>
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>
              Tienes {myActive.length} tarea{myActive.length !== 1 ? 's' : ''} activa
              {myActive.length !== 1 ? 's' : ''}
            </p>
          </div>
          ...
          {myUrgent.length > 0 && (
            ...
            >
              <p style={{ fontSize: 13, fontWeight: 700, color: '#fca5a5', marginBottom: 10 }}>
                <Icon n="vencida" size={13} style={{ marginRight: 4 }} /> Requieren atención
                inmediata
              </p>
                ...
                >
                  {t.order_number && (
                    <span style={{ fontSize: 11, color: '#fca5a5', fontWeight: 700 }}>
                      #{String(t.order_number).padStart(4, '0')}
                    </span>
                  )}
                  ...
                  {(() => {
                    ...
                    return (
                      <span style={{ color: dr.color, fontWeight: 700, fontSize: 11 }}>
                        {dr.label}
                      </span>
                    )
                  })()}
            ...
            {myActive.length === 0 ? (
              <div className="empty">
                <div style={{ opacity: 0.35, marginBottom: 8 }}>
                  <Icon n="check" size={44} color="var(--s-completada)" />
                </div>
                <p>Sin tareas pendientes</p>
                <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>
                  Cuando te asignen una orden nueva, aparecerá aquí.
                </p>
              </div>
            ) : (
              myActive.map((t) => {
                const team = teams.find((x) => x.id === t.team_id),
                  dr = fmtDateRelative(t.due_date, t.status)
                return (
                  <div
                    key={t.id}
                    ...
                    style={{
                      cursor: 'pointer',
                      borderLeft: `3px solid ${statusColor[t.status] || 'var(--border2)'}`,
                      marginBottom: 8,
                    }}
                    onClick={() => onOpenTask && onOpenTask(t)}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <span className={`pill ${statusPill[t.status] || 'pill-gray'}`}>
                        {statusLabel[t.status]}
                      </span>
                    </div>
                  </div>
      ...
      {(isDir || isCuentas) && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            {(isDir || isCuentas) && (
              <button className="quick-action" onClick={() => onNavigate('crear')}>
            ...
            <button
              ...
              style={{ borderColor: overdue.length > 0 ? 'rgba(232,93,93,.3)' : 'var(--border)' }}
            >
              <div
                className="quick-action-icon"
                style={{ background: 'rgba(232,93,93,.1)', position: 'relative' }}
              >
              ...
              <span
                style={{
                  fontSize: 11,
                  fontWeight: 600,
                  color: overdue.length > 0 ? 'var(--s-vencida)' : 'inherit',
                }}
              >
                Vencidas{overdue.length > 0 ? ` (${overdue.length})` : ''}
              </span>
            </button>
            ...
            {isDir && (
              <button className="quick-action" onClick={() => onNavigate('desempeno')}>
          ...
          <div className="card" style={{ marginBottom: 16, padding: '12px 18px' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                ...
                <StatCard
                  val={forReview.length}
                  label="en revisión"
                  color="var(--s-revision)"
                  onClick={() => onNavigate('ordenes', 'en_revision')}
                />
                <StatCard
                  val={overdue.length}
                  label="vencidas"
                  color={overdue.length > 0 ? 'var(--s-vencida)' : 'var(--s-completada)'}
                  onClick={() => onNavigate('ordenes', 'vencida')}
                />
                {onPause.length > 0 && (
                  <StatCard
                    ...
                    label="en pausa"
                    color="var(--s-pausa)"
                    onClick={() => onNavigate('ordenes', 'en_pausa')}
                  />
                )}
            ...
            (() => {
              ...
              return (
                <div className="card fade-in" style={{ marginBottom: 16 }}>
                  ...
                  >
                    <h3 style={{ fontSize: 15, fontWeight: 700 }}>
                      <Icon n="marca" size={14} style={{ marginRight: 6 }} />
                      Mis marcas
                    </h3>
                    <span
                      style={{
                    ...
                    >
                      {brands.length} con trabajo activo
                    </span>
                  </div>
                  <div
                    style={{
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            background: 'var(--bg3)',
                            borderRadius: 10,
                            border: `1px solid ${hasAlert ? 'rgba(232,93,93,.35)' : 'var(--border)'}`,
                            cursor: 'pointer',
                            transition: '.13s',
                            position: 'relative',
                            overflow: 'hidden',
                        ...
                        >
                          ...
                          >
                            {b.revision > 0 && (
                              ...
                              >
                                <Icon n="revision" size={9} /> {b.revision} en revisión
                              </span>
                            )}
                            {b.vencidas > 0 && (
                              <span
                              ...
                              >
                                <Icon n="alerta" size={9} /> {b.vencidas} vencida
                                {b.vencidas > 1 ? 's' : ''}
                              </span>
                            )}
                            {b.revision === 0 && b.vencidas === 0 && (
                              <span
                                style={{
                                  fontSize: 10,
                                  color: 'var(--muted)',
          ...
          >
            <div className="card">
              ...
              >
                <h3 style={{ fontSize: 15, fontWeight: 700 }}>
                  <Icon n="revision" size={14} style={{ marginRight: 6 }} />
                  Cola de revisión
                </h3>
              ...
              {forReview.length === 0 ? (
                <p
                  style={{ fontSize: 13, color: 'var(--muted)', textAlign: 'center', padding: 16 }}
                >
                ...
                forReview.slice(0, 5).map((t) => {
                  const u = users.find(
                    (x) =>
                      x.id === (Array.isArray(t.assigned_to) ? t.assigned_to[0] : t.assigned_to)
                  )
                  return (
                    <div
                      key={t.id}
              ...
              {forReview.length > 5 && (
                ...
                >
                  Ver {forReview.length - 5} más →
                </button>
              )}
            </div>
            <div
              className="card"
              style={{ borderColor: overdue.length > 0 ? 'rgba(239,68,68,.3)' : 'var(--border)' }}
            >
              <div
                style={{
                  display: 'flex',
              ...
              >
                <h3
                  style={{
                    fontSize: 15,
                    fontWeight: 700,
                    color: overdue.length > 0 ? '#fca5a5' : 'var(--text)',
                  }}
                >
                  <Icon n="vencida" size={14} style={{ marginRight: 6 }} /> Vencidas
                </h3>
                <span style={{ fontSize: 12, color: 'var(--muted)' }}>{overdue.length}</span>
              </div>
              {overdue.length === 0 ? (
                <p
                  style={{ fontSize: 13, color: 'var(--green)', textAlign: 'center', padding: 16 }}
                >
                  ✓ Todo al día
                </p>
              ) : (
                overdue.slice(0, 5).map((t) => {
                  ...
                  const days = Math.ceil(
                    (new Date() - new Date(t.due_date + 'T00:00:00')) / (1000 * 60 * 60 * 24)
                  )
                  return (
                    <div
                      key={t.id}
                      style={{
                        ...
                        gap: 10,
                        padding: '8px 0',
                        borderBottom: '1px solid rgba(240,107,107,.1)',
                        cursor: 'pointer',
                        borderRadius: 5,
                        transition: '.12s',
                      }}
                      onClick={() => onOpenTask && onOpenTask(t)}
                      onMouseEnter={(e) =>
                        (e.currentTarget.style.background = 'rgba(240,107,107,.04)')
                      }
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                    >
                      {t.order_number && (
                      ...
                      <span
                        style={{
                          fontSize: 11,
                          background: 'rgba(240,107,107,.12)',
                          color: 'var(--red)',
                          fontWeight: 700,
                          flexShrink: 0,
                          padding: '2px 7px',
              ...
              {overdue.length > 0 && (
                <button
                  ...
                  style={{
                    ...
                    marginTop: 10,
                    padding: '7px',
                    background: 'rgba(240,107,107,.08)',
                    border: '1px solid rgba(240,107,107,.2)',
                    borderRadius: 6,
                    color: 'var(--red)',
                    fontSize: 12,
                    cursor: 'pointer',
          ...
          {isDir && (
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              >
                <div>
                  <h3 style={{ fontSize: 15, fontWeight: 700 }}>
                    <Icon n="semaforo" size={14} style={{ marginRight: 6 }} />
                    Semáforo de equipos
                  </h3>
                  <p
                    style={{
                  ...
                  >
                    <span style={{ color: 'var(--s-completada)' }}>●</span> Libre ·{' '}
                    <span style={{ color: 'var(--yellow)' }}>●</span> Cargado ≥4/pers ·{' '}
                    <span style={{ color: 'var(--s-vencida)' }}>●</span> Urgente o sobrecarga
                    ≥7/pers
                  </p>
                </div>
                <button
              ...
              >
                {teams.map((team) => {
                  ...
                  const health =
                    teamOverdue > 0 || overloaded > 0 ? 'red' : avgLoad >= 4 ? 'yellow' : 'green'
                  const healthColor = {
                    red: 'var(--load-crit)',
                  ...
                  const healthLabel = {
                    red:
                      teamOverdue > 0
                        ? `${teamOverdue} vencida${teamOverdue > 1 ? 's' : ''}`
                        : overloaded > 0
                  ...
                  return (
                    <div
                      key={team.id}
                      onClick={() => onNavigate('ordenes', { teamId: team.id })}
          ...
          <div className="card">
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>
              <Icon n="carga" size={14} style={{ marginRight: 6 }} />
            ...
            {workload.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px 16px' }}>
                ...
                {isDir && (
                  ...
                  >
                    Ir a Administración para invitar →
                  </button>
                )}
              </div>
            ) : (
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: '6px 8px',
                      borderRadius: 8,
                      cursor: isDir || isCuentas ? 'pointer' : 'default',
                      transition: '.13s',
                    }}
                    onClick={() => {
                      if ((isDir || isCuentas) && onViewUser) onViewUser(w)
                    }}
                    onMouseEnter={(e) => {
                      if (isDir || isCuentas) e.currentTarget.style.background = 'var(--bg3)'
                    }}
                    onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                  >
                    <div
                    ...
                    <div style={{ minWidth: 60, textAlign: 'right', flexShrink: 0 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color }}>{w.active}</span>
                    ...
                    {(isDir || isCuentas) && (
                      <span
                        style={{ fontSize: 11, color: 'var(--muted)', flexShrink: 0, opacity: 0.5 }}
                      >
                        →
                      </span>
                    )}
