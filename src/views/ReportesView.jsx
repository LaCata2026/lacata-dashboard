...
function taskHadActivityInRange(t, range) {
  ...
  if (t.started_at && new Date(t.started_at) >= range.from && new Date(t.started_at) <= range.to)
    return true
  if (t.created_at && new Date(t.created_at) >= range.from && new Date(t.created_at) <= range.to)
    return true
  return false
}
...
function PeriodBar({ period, offset, onChange }) {
  ...
  const label =
    from.toLocaleDateString('es-GT', opts) + ' – ' + to.toLocaleDateString('es-GT', opts)
  return (
...
function TabColaborador({ tasks, users, teams, range }) {
  ...
  return (
    <div>
      ...
      {rows.length === 0 && (
        <p style={{ textAlign: 'center', color: 'var(--muted)', padding: 48, fontSize: 14 }}>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 120 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--muted)' }}>
                  ...
                  {marcas.length > 0 &&
                    ' · ' + marcas.slice(0, 3).join(', ') + (marcas.length > 3 ? '…' : '')}
                </div>
              </div>
            ...
            return (
              ...
              >
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)' }}>
                    {names || 'Sin asignar'} · {fmtD(t.created_at)}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span
      ...
      >
        {rows.map((r, i) => (
          ...
          >
            ...
            <div
              style={{ height: 4, background: 'var(--bg3)', borderRadius: 2, overflow: 'hidden' }}
            >
              <div
                style={{
...
export default function ReportesView({ tasks, users, teams, onBack }) {
  ...
  return (
    <div>
      ...
      {/* Tab content */}
      {tab === 'colaborador' && (
        <TabColaborador tasks={tasks} users={users} teams={teams} range={range} />
      )}
