    ...
    <div className="fade-in">
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <button onClick={prevMonth} style={{background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:7,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--muted2)",transition:".13s"}}
          ...
          <h3 style={{fontSize:17,fontWeight:800,fontFamily:"var(--font-display)",letterSpacing:"-.02em",minWidth:160,textAlign:"center"}}>{monthNames[calMonth]} {calYear}</h3>
          <button onClick={nextMonth} style={{background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:7,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--muted2)",transition:".13s"}}
            onMouseEnter={e=>e.currentTarget.style.color="var(--text)"} onMouseLeave={e=>e.currentTarget.style.color="var(--muted2)"}>
          ...
          <button onClick={()=>{setCalMonth(today.getMonth());setCalYear(today.getFullYear());}}
            style={{fontSize:11,padding:"4px 10px",borderRadius:6,background:"var(--accent-dim)",color:"var(--accent)",border:"1px solid rgba(232,197,71,.25)",cursor:"pointer",fontFamily:"var(--font-body)",fontWeight:700}}>
            Hoy
          </button>
        </div>
        {/* Month summary pills */}
        <div style={{display:"flex",gap:8}}>
          <span style={{fontSize:11,padding:"3px 10px",borderRadius:999,background:"var(--bg3)",color:"var(--muted2)",fontFamily:"var(--font-mono)"}}>
            <span style={{color:"var(--accent)",fontWeight:700}}>{monthTasks.length}</span> entrega{monthTasks.length!==1?"s":""}
          </span>
          {overdueMonth>0&&<span style={{fontSize:11,padding:"3px 10px",borderRadius:999,background:"var(--s-vencida-bg)",color:"var(--s-vencida)",fontFamily:"var(--font-mono)",fontWeight:700}}>⚠ {overdueMonth} vencida{overdueMonth!==1?"s":""}</span>}
          {doneMonth>0&&<span style={{fontSize:11,padding:"3px 10px",borderRadius:999,background:"var(--s-completada-bg)",color:"var(--s-completada)",fontFamily:"var(--font-mono)"}}>✓ {doneMonth} completada{doneMonth!==1?"s":""}</span>}
        </div>
      </div>
      ...
      <div className="cal-grid">
        {cells.map(cell=>{
          ...
          return(
            <div key={dateStr} className={`cal-cell${isToday?" today":""}${dayTasks.length>0?" has-events":""}`}
      ...
      <div style={{display:"flex",gap:16,marginTop:16,flexWrap:"wrap",alignItems:"center"}}>
        <span style={{fontSize:11,color:"var(--muted)",fontFamily:"var(--font-mono)"}}>Estado de fecha límite:</span>
        {[
          {s:"en_progreso",l:"En progreso"},{s:"en_revision",l:"En revisión"},
          {s:"pendiente",l:"Pendiente"},{s:"vencida",l:"Vencida"},{s:"completada",l:"Completada"},
        ].map(({s,l})=>(
          <span key={s} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:"var(--muted2)"}}>
