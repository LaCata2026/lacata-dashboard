...
const eff = (est, real) => {
  const e = Number(est),
    r = Number(real)
  if (!e || !r) return null
  return Math.min(999, Math.round((e / r) * 100))
}
const effColor = (e) =>
  e == null ? 'var(--muted)' : e >= 90 ? 'var(--green)' : e >= 70 ? 'var(--yellow)' : 'var(--red)'
const effLabel = (e) => (e == null ? '—' : e + '%')

function getRangeLabel(period, offset) {
  const now = new Date()
  if (period === 'todo')
    return { from: new Date(2000, 0, 1), to: new Date(2099, 11, 31, 23, 59, 59, 999), all: true }
  let from, to
  if (period === 'semana') {
    const ref = new Date(now)
    ref.setDate(ref.getDate() + offset * 7)
    const day = ref.getDay(),
      diff = ref.getDate() - day + (day === 0 ? -6 : 1)
    from = new Date(ref)
    from.setDate(diff)
...
function taskHadActivity(t, range) {
  ...
  if (
    hist.some((e) => {
      ...
      if (iso) return inRange(new Date(iso[0]))
      const loc = e.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/)
      if (loc)
        return inRange(new Date(`${loc[3]}-${loc[2].padStart(2, '0')}-${loc[1].padStart(2, '0')}`))
      return false
    })
  )
    return true
...
function filterByRange(tasks, range) {
  ...
  return tasks.filter((t) => taskHadActivity(t, range))
}
...
function PeriodBar({ period, offset, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
      <div
        style={{ display: 'flex', background: 'var(--bg3)', borderRadius: 8, padding: 3, gap: 3 }}
      >
        {[
          ['todo', 'Todo'],
          ['mes', 'Mes'],
        ...
        <>
          ...
          <button
            ...
            style={{
              ...
              color: offset >= 0 ? 'var(--border)' : 'var(--text)',
              fontSize: 13,
            }}
          >
            ›
...
function Chip({ label, value, color, sub }) {
  return (
    <div style={{ textAlign: 'center', minWidth: 60 }}>
      ...
      {sub && (
        <div
          style={{ fontSize: 10, color: color || 'var(--muted)', fontFamily: 'var(--font-mono)' }}
        >
          {sub}
        </div>
      )}
...
function BarChart({ data, maxVal, colorFn, labelKey, valueKey, valueSuffix = '' }) {
  ...
  return (
    <svg
      width="100%"
      viewBox={`0 0 ${LABEL_W + BAR_W + VALUE_W + 16} ${h}`}
          ...
          <g key={d[labelKey] || i}>
            ...
            {barW > 0 && (
              <rect
                ...
                rx={4}
                fill={color}
                style={{ transition: 'width .6s cubic-bezier(.4,0,.2,1)' }}
              />
            )}
            <text
              x={LABEL_W + BAR_W + 8}
...
function StackedBar({ comp, actv, venc, total }) {
  ...
  const W = 100,
    wComp = Math.round((comp / total) * W),
    wVenc = Math.round((venc / total) * W),
    wActv = W - wComp - wVenc
  return (
    <div
      style={{
    ...
    >
      {wComp > 0 && (
        <div style={{ width: wComp + '%', background: 'var(--green)', transition: 'width .5s' }} />
      )}
      {wActv > 0 && (
        <div
          style={{
      ...
      {wVenc > 0 && (
        <div style={{ width: wVenc + '%', background: 'var(--red)', transition: 'width .5s' }} />
      )}
    </div>
  )
}
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {teamTasks.length} activa{teamTasks.length !== 1 ? 's' : ''}
                </span>
                {overdueCount > 0 && (
                  <span
                    style={{
                ...
                {visibleMembers.map((m) => {
                  ...
                  return (
                    ...
                    >
                      {/* Fila del miembro */}
                      <div
                        onClick={() => setOpenMembers((o) => ({ ...o, [m.id]: !o[m.id] }))}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: 8,
    ...
    () =>
      colabs
        .map((u) => {
          ...
          const score =
            comp.length * 10 - venc.length * 15 + (e != null ? Math.min(e, 150) / 50 : 0)
          return { u, mt, comp, venc, actv, hrsE, hrsR, e, marcas, score }
        })
        .filter((r) => r.mt.length > 0)
        .sort((a, b) => b.score - a.score),
    [filtered]
  )
  const maxHrs = Math.max(...rows.map((r) => r.hrsR), 1)
  const top = rows[0],
    bot = rows.length > 1 ? rows[rows.length - 1] : null
  function doExport() {
    const hdr = [
      'Ranking',
      'Colaborador',
    ...
    const data = rows.map(({ u, mt, comp, venc, actv, hrsE, hrsR, e, marcas }, i) => {
    ...
    exportCSV(
      [hdr, ...data],
      'LaCata_Colaboradores_' + new Date().toISOString().split('T')[0] + '.csv'
    )
  }
  if (detail) {
    const { u, mt } = detail
    ...
    const hrsE = mt.reduce((s, t) => s + Number(t.hours || 0), 0),
      hrsR = mt.reduce((s, t) => s + Number(t.hours_real || 0), 0)
    return (
      <div>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            <Chip
              label="COMPLET."
              value={mt.filter((t) => t.status === 'completada').length}
              color="var(--green)"
            />
            ...
            <Chip
              label="EFIC."
              value={effLabel(eff(hrsE, hrsR))}
              color={effColor(eff(hrsE, hrsR))}
            />
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {sorted.map((t) => {
            ...
            return (
              <div
                ...
                onMouseEnter={(ev) => {
                  if (onOpenTask) ev.currentTarget.style.borderColor = 'var(--accent)'
                }}
        ...
        <div className="card fade-in" style={{ marginBottom: 20 }}>
          ...
          >
            ...
            <span style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
              Mejor → peor · completadas vs vencidas
            </span>
          </div>
          <p
            style={{
          ...
          >
            Posición = tareas completadas, penalizando vencidas. No es por horas trabajadas.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {rows.map((r, i) => {
              ...
              return (
                <div
                  key={u.id}
                  onClick={() => setDetail({ u: r.u, mt: r.mt })}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <Chip
                      label="VENCIDAS"
                      value={venc.length}
                      color={venc.length > 0 ? 'var(--red)' : 'var(--muted)'}
                    />
                    <Chip label="HRS REAL" value={fmtH(hrsR)} color="var(--blue)" />
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                ...
                <div
                  style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}
                >
                  {top.comp.length} completadas
                  {top.venc.length > 0 ? ` · ${top.venc.length} venc.` : ''} · {effLabel(top.e)}
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                ...
                <div
                  style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}
                >
                  {bot.comp.length} completadas
                  {bot.venc.length > 0 ? ` · ${bot.venc.length} venc.` : ''} · {effLabel(bot.e)}
                </div>
              </div>
            </div>
          </div>
...
function TabEquipos({ tasks, users, teams, range }) {
    ...
    () =>
      teams
        .map((team) => {
          ...
          const comp = mt.filter((t) => t.status === 'completada'),
            venc = mt.filter((t) => t.status === 'vencida')
          const hrsE = mt.reduce((s, t) => s + Number(t.hours || 0), 0),
            hrsR = mt.reduce((s, t) => s + Number(t.hours_real || 0), 0)
          const e = eff(hrsE, hrsR),
            avgHrsPerOrder = mt.length > 0 ? hrsR / mt.length : 0
  ...
  const topTeam = rows[0],
    botTeam = [...rows].sort((a, b) => a.mt.length - b.mt.length)[0]
  function doExport() {
    const hdr = [
      'Equipo',
      'Miembros',
    ...
    const data = rows.map(({ team, members, mt, comp, venc, hrsE, hrsR, e, avgHrsPerOrder }) => [
      team.name,
      members.length,
      mt.length,
      comp.length,
      ...
      <div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>{members.length} miembros</div>
          </div>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
    ...
    <div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 12, fontSize: 11, fontFamily: 'var(--font-mono)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <div
                  style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--green)' }}
                />{' '}
                Completadas
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
          {topTeam && (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                {topTeam.mt.length} órdenes · {topTeam.comp.length} completadas ·{' '}
                {fmtH(topTeam.hrsR)}
              </div>
            </div>
          )}
          {botTeam && botTeam.team.id !== topTeam?.team.id && (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                {botTeam.mt.length} órdenes · {botTeam.comp.length} completadas ·{' '}
                {fmtH(botTeam.hrsR)}
              </div>
            </div>
          )}
      ...
      >
        {rows.map(({ team, members, mt, comp, venc, hrsR, e, avgHrsPerOrder }) => (
          <div
            key={team.id}
            onClick={() => setDetail({ team, members, mt })}
            style={{
              background: 'var(--bg2)',
              border: '1px solid var(--border)',
              borderRadius: 12,
              ...
              cursor: 'pointer',
              transition: '.13s',
              borderLeft: `3px solid ${teamColor(team)}`,
            }}
          ...
          >
            ...
            <StackedBar
              comp={comp.length}
              actv={mt.filter((t) => t.status !== 'completada' && t.status !== 'vencida').length}
              venc={venc.length}
              total={mt.length}
            />
            <div
            ...
            {venc.length > 0 && (
              <div style={{ marginTop: 8, fontSize: 11, color: 'var(--red)', fontWeight: 600 }}>
                {venc.length} vencida{venc.length !== 1 ? 's' : ''} este período
              </div>
            )}
          </div>
        ))}
...
function TabMarcas({ tasks, users, teams, range }) {
    ...
    return Object.values(map)
      .map((r) => {
        const hrsE = r.tasks.reduce((s, t) => s + Number(t.hours || 0), 0),
          hrsR = r.tasks.reduce((s, t) => s + Number(t.hours_real || 0), 0),
          comp = r.tasks.filter((t) => t.status === 'completada').length,
          venc = r.tasks.filter((t) => t.status === 'vencida').length,
          changes = r.tasks.reduce((s, t) => s + Number(t.changes || 0), 0),
          e = eff(hrsE, hrsR)
  ...
  const topMarca = rows[0],
    botMarca = rows.length > 1 ? rows[rows.length - 1] : null,
    mostChanges = [...rows].sort((a, b) => b.changes - a.changes)[0],
    maxHrs = Math.max(...rows.map((r) => r.hrsR), 1)
  function doExport() {
    const hdr = [
      'Marca',
      'Órdenes',
      ...
      <div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>
              {mt.length} órdenes en este período
            </div>
          </div>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <Chip label="ÓRDENES" value={mt.length} color="var(--accent)" />
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {fmtH(c.hrsR)} · {c.tasks.length} tareas
                      </div>
                    </div>
                  </div>
                )
            ...
            {rows.length >= 2 && topMarca && botMarca && (
              <div className="card fade-in">
                ...
                {[
                  {
                    label: 'Órdenes',
                    top: topMarca.tasks.length,
                    bot: botMarca.tasks.length,
                    color: 'var(--accent)',
                  },
                  {
                    label: 'Completadas',
                  ...
                  {
                    label: 'Cambios',
                    top: topMarca.changes,
                    bot: botMarca.changes,
                    color: 'var(--yellow)',
                  },
                  ...
                  return (
                    <div key={label} style={{ marginBottom: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                        ...
                        >
                          {typeof top === 'number' && top % 1 !== 0 ? top.toFixed(1) : top}
                        </span>
                        <div
                          style={{
                            flex: bot / mx,
            ...
            {mostChanges && mostChanges.changes > 0 && (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>
                  {mostChanges.changes} cambios en {mostChanges.tasks.length} órdenes
                </div>
              </div>
            )}
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {rows
                    .filter((r) => r.venc > 0)
                    .map((r) => (
                      <div key={r.marca} style={{ fontSize: 13, fontWeight: 600 }}>
                        {r.marca}{' '}
                        <span
                          style={{
                            color: 'var(--red)',
        ...
        {rows.map((r, i) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
                {r.colabs.size} colab{r.colabs.size !== 1 ? 's' : ''}
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            >
              <div
                style={{
                  width: (r.comp / Math.max(r.tasks.length, 1)) * 100 + '%',
                  height: '100%',
    ...
    () =>
      filtered
        .filter((t) => {
          ...
          if (q) {
            const names = assignedOf(t)
              .map((id) => users.find((u) => u.id === id)?.name || '')
              .join(' ')
              .toLowerCase()
            const orderN = t.order_number ? 'ac-' + String(t.order_number).padStart(4, '0') : ''
            if (![t.title, names, t.marca || '', orderN].some((s) => s.toLowerCase().includes(q)))
              return false
          }
    ...
    <div>
      ...
      >
        <input
          ...
          placeholder="Buscar orden, colaborador, marca..."
          style={{
            flex: 1,
            minWidth: 200,
            padding: '7px 12px',
      ...
      {rows.length === 0 && (
        <p style={{ textAlign: 'center', color: 'var(--muted)', padding: 48 }}>Sin resultados.</p>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {rows.map((t) => {
          const assigned = assignedOf(t)
            .map((id) => users.find((u) => u.id === id))
            .filter(Boolean)
          const team = teams.find((x) => x.id === t.team_id),
            e = eff(t.hours, t.hours_real)
          const isOverdue = t.status === 'vencida',
            isComp = t.status === 'completada'
          return (
            <div
              key={t.id}
              style={{
            ...
            >
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {t.due_date && (
                    <span
                      style={{ fontSize: 11, color: isOverdue ? 'var(--red)' : 'var(--muted)' }}
                    >
                      · límite {fmtD(t.due_date)}
                    </span>
                  )}
                </div>
              </div>
...
export default function IntelView({
  ...
  const myTeamIds = isCuentas
    ? Array.isArray(myProfile?.team_ids) && myProfile.team_ids.length > 0
      ? myProfile.team_ids
      : [myProfile?.team_id].filter(Boolean)
    : null
  const visibleTasks =
  ...
  return (
    <div>
      ...
      {tab === 'equipos' && (
        <TabEquipos tasks={visibleTasks} users={users} teams={teams} range={range} />
      )}
      {tab === 'marcas' && (
        <TabMarcas tasks={visibleTasks} users={users} teams={teams} range={range} />
      )}
      {tab === 'ordenes' && (
        <TabOrdenes
          tasks={visibleTasks}
