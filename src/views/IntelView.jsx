...
function getRangeLabel(period,offset){
  const now=new Date()
  // Período "todo": sin filtro de fecha — abarca toda la historia
...
/* ═══════════════════════════════════════════
   TAB — COLABORADORES
   Ranking de desempeño: mejor → peor
   Score = completadas - vencidas, desempate por eficiencia
═══════════════════════════════════════════ */
function TabDesempeno({tasks,users,teams,range,onOpenTask}){
  ...
  const rows=useMemo(()=>colabs.map(u=>{
    ...
    // Score de desempeño: completadas pesan +, vencidas pesan --,
    // eficiencia da bonus pequeño. Más horas NO = mejor desempeño.
  ...
  function doExport(){
    ...
    const data=rows.map(({u,mt,comp,venc,actv,hrsE,hrsR,e,marcas},i)=>{
      ...
      return[i+1,u.name,team?.name||"—",mt.length,comp.length,venc.length,actv.length,fmtH(hrsE),fmtH(hrsR),effLabel(e),marcas.join(", ")||"—"]
    })
          ...
          {sorted.map(t=>{
            ...
            return(
              <div key={t.id} onClick={()=>onOpenTask&&onOpenTask(t)}
                ...
                onMouseEnter={ev=>{if(onOpenTask)ev.currentTarget.style.borderColor="var(--accent)"}}
            ...
            {rows.map((r,i)=>{
              ...
              return(
                <div key={u.id} onClick={()=>setDetail({u:r.u,mt:r.mt})}
                  ...
                  onMouseEnter={ev=>ev.currentTarget.style.borderColor=u.avatar_color||"var(--accent)"}
                  onMouseLeave={ev=>ev.currentTarget.style.borderColor="var(--border)"}>
                  <div style={{minWidth:34,textAlign:"center"}}>
                    {medal?<span style={{fontSize:18}}>{medal}</span>:<span style={{fontSize:14,fontWeight:800,color:rankColor,fontFamily:"var(--font-display)"}}>{i+1}</span>}
                  </div>
                  ...
                  <div style={{flex:1,minWidth:100}}>
                    ...
                    <div style={{height:6,background:"var(--bg2)",borderRadius:3,overflow:"hidden"}}>
                      <div style={{width:barPct+"%",height:"100%",background:u.avatar_color||"var(--accent)",borderRadius:3,transition:"width .6s cubic-bezier(.4,0,.2,1)"}}/>
                    </div>
                  </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
          <div style={{background:"var(--bg2)",border:"1px solid var(--green)",borderRadius:12,padding:"14px 16px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <Av u={top.u} size={28}/>
              <div><div style={{fontSize:14,fontWeight:700}}>{top.u.name}</div>
              <div style={{fontSize:11,color:"var(--muted)",fontFamily:"var(--font-mono)"}}>{top.comp.length} completadas{top.venc.length>0?` · ${top.venc.length} venc.`:""} · {effLabel(top.e)}</div></div>
            </div>
          </div>
          <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:12,padding:"14px 16px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <Av u={bot.u} size={28}/>
              <div><div style={{fontSize:14,fontWeight:700}}>{bot.u.name}</div>
              <div style={{fontSize:11,color:"var(--muted)",fontFamily:"var(--font-mono)"}}>{bot.comp.length} completadas{bot.venc.length>0?` · ${bot.venc.length} venc.`:""} · {effLabel(bot.e)}</div></div>
            </div>
          </div>
        </div>
      )}
...
export default function IntelView({tasks,users,teams,onBack,me,profile,token,onRefresh,onLoadHistory,initialUser,onNavigate,onOpenTask}){
  ...
  // El histórico completo siempre se necesita aquí — cargarlo al montar
  useEffect(()=>{
    if(historyLoaded||!onLoadHistory)return
    setHistoryLoaded(true);onLoadHistory()
