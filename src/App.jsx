...
function UpdateBanner() {
  return (
    ...
    >
      ...
      <span style={{ fontSize: 11, opacity: 0.7 }}>Recarga para no perder funcionalidad</span>
    </div>
  )
}
...
export default function App() {
  ...
  const [isDark, setIsDark] = useState(() => {
    const s = localStorage.getItem('lc_theme')
  ...
  useEffect(() => {
    ...
    async function refreshProfile() {
      try {
        ...
        if (typeof freshProfile.team_ids === 'string') {
          try {
            freshProfile.team_ids = JSON.parse(freshProfile.team_ids)
          } catch {
            freshProfile.team_ids = []
          }
        }
  ...
  const isInvite =
    (typeFromHash === 'invite' ||
      ...
      typeFromQuery === 'invite' ||
      typeFromQuery === 'recovery' ||
      !!tokenFromHash ||
      !!tokenFromQuery) &&
    (tokenFromHash || tokenFromQuery)
  ...
  return (
    <>
      ...
      {session && expired && (
        <SessionExpiredModal onContinue={handleContinue} onLogout={handleLogout} />
      )}
      {session ? (
        <Dashboard
          session={session}
