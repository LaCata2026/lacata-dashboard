...
function extractToken() {
  ...
  if (hash) {
    ...
    if (t && t.split('.').length === 3)
      return { token: t, refresh: hashParams.get('refresh_token'), source: 'hash' }
  }
  ...
  if (search) {
    ...
    if (t && t.split('.').length === 3)
      return { token: t, refresh: searchParams.get('refresh_token'), source: 'query' }
  ...
  try {
    ...
    console.warn('[SetPassword] error reading cookies:', e)
  }
...
export default function SetPassword() {
  ...
  useEffect(() => {
    ...
    console.log(
      '[SetPassword] Token extraction:',
      info ? { source: info.source, hasToken: !!info.token, hasCode: !!info.code } : 'none'
    )
    setTokenInfo(info)
  }, [])
        ...
        <div className="login-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--muted)' }}>Enlace inválido</p>
            </div>
          </div>
          <div
            style={{
  ...
  async function savePassword() {
    ...
    if (!/[A-Z]/.test(pw)) return setErr('Debe incluir al menos una letra mayúscula')
    if (!/[0-9]/.test(pw)) return setErr('Debe incluir al menos un número')
    ...
    try {
      ...
      if (tokenInfo.source === 'pkce' && tokenInfo.code) {
        ...
        if (!exchangeR.ok || !exchangeD.access_token) {
          throw new Error(
            'No se pudo verificar el enlace. Por favor abre el enlace más reciente del correo.'
          )
        }
      ...
      if (!r.ok || d.error || d.code)
        throw new Error(
          d.msg || d.error_description || d.error || 'No se pudo guardar la contraseña'
        )
      ...
      const fallbackName = email
        ? email.split('@')[0]
        : 'usuario_' + Math.random().toString(36).slice(2, 6)
      ...
      if (!loginR.ok || !loginD.access_token) {
        throw new Error(
          'La contraseña se guardó pero no se pudo iniciar sesión automáticamente. Por favor ve al login y entra con tu correo y contraseña.'
        )
      }
      ...
      <div className="login-card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--muted)' }}>Crea tu contraseña</p>
          </div>
        </div>
        <p style={{ color: 'var(--muted2)', fontSize: 14, marginBottom: 20, lineHeight: 1.5 }}>
          Bienvenido a Agarrate Catalina Creative Ops. Crea tu contraseña para ingresar.
        </p>
        ...
        {err && (
          ...
          >
            <Icon n="alerta" size={12} style={{ marginRight: 4 }} />
            {err}
          </div>
        )}
        <button
