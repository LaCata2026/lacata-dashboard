      ...
      <div className="spotlight-box" onKeyDown={onKey}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 16px' }}>
          ...
          <input
            ...
            placeholder="Buscar orden, número AC-0001, marca..."
          />
          <span
            style={{
              fontSize: 11,
        ...
        {results.length > 0 && (
          <div style={{ maxHeight: 360, overflowY: 'auto' }}>
            {results.map((r, i) => (
              <div
                key={r.t.id}
                className={`spotlight-result${sel === i ? ' active' : ''}`}
                onClick={() => go(r)}
              ...
              >
                ...
                >
                  AC-{String(r.t.order_number || 0).padStart(4, '0')}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--muted)' }}>
                    {r.names} · {r.team?.name || 'Sin equipo'}
                  </p>
                </div>
                <span className={`pill ${statusPill[r.t.status] || 'pill-gray'}`}>
                  {statusLabel[r.t.status]}
                </span>
              </div>
            ))}
          </div>
        ...
        {q.trim() && results.length === 0 && (
          <div
            style={{ padding: '20px', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}
          >
        ...
        {!q.trim() && (
          <div style={{ padding: '12px 16px', display: 'flex', gap: 16 }}>
            {[
              { icon: 'ordenes', label: 'Nombre o descripcion' },
              { icon: 'buscar', label: 'AC-0001' },
              { icon: 'marca', label: 'Marca o equipo' },
              { icon: 'persona', label: 'Responsable' },
            ].map((h, i) => (
              <span
                key={i}
                style={{
              ...
              >
                <Icon n={h.icon} size={13} style={{ marginRight: 2 }} />
                {h.label}
              </span>
            ))}
          </div>
