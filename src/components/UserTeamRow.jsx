...
export default function UserTeamRow({
  u,
  teams,
  tasks,
  token,
  ...
  const [localIds, setLocalIds] = useState(() =>
    Array.isArray(u.team_ids) && u.team_ids.length > 0 ? u.team_ids : u.team_id ? [u.team_id] : []
  )
  useEffect(() => {
    const fresh =
      Array.isArray(u.team_ids) && u.team_ids.length > 0 ? u.team_ids : u.team_id ? [u.team_id] : []
  ...
  async function toggle(teamId) {
    const next = localIds.includes(teamId)
      ? localIds.filter((x) => x !== teamId)
      : [...localIds, teamId]
    setLocalIds(next)
    try {
  ...
  const displayName =
    u.name && !u.name.startsWith('usuario_') ? u.name : u.email?.split('@')[0] || 'Sin nombre'
    ...
    <div style={{ borderTop: '1px solid var(--border)', padding: '10px 12px' }}>
          ...
          {isCollab && active > 0 && (
            <span
              style={{
                ...
                top: -4,
                right: -4,
                background: overdue > 0 ? 'var(--s-vencida)' : ROLE_COLOR.colaborador,
                color: '#fff',
                fontSize: 9,
                fontWeight: 700,
                borderRadius: '50%',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <p
              style={{
                fontSize: 13,
                fontWeight: 600,
                color: nameIsAuto ? 'var(--muted)' : 'var(--text)',
                fontStyle: nameIsAuto ? 'italic' : 'normal',
              }}
            >
              {displayName}
            </p>
      ...
      {(isCuentas || isCollab) && (
        ...
        >
          ...
          {isCuentas && localIds.length === 0 && (
            <span style={{ fontSize: 11, color: 'var(--muted)', fontStyle: 'italic' }}>
              Sin restricción — ve todos
            </span>
          )}
          {isCollab && localIds.length === 0 && (
            <span style={{ fontSize: 11, color: 'var(--muted)', fontStyle: 'italic' }}>
