...
export function Av({ u, size = 32 }) {
  ...
  return (
    <div
      className="avatar"
      style={{ width: size, height: size, background: bg, fontSize: size * 0.33, color: '#fff' }}
    >
      {u?.initials || '?'}
    </div>
  )
...
export function ActiveTimer({ startedAt, hoursReal }) {
  ...
  useEffect(() => {
    if (!startedAt) return
    const update = () =>
      setElapsed(Math.max(0, (Date.now() - new Date(startedAt).getTime()) / 3600000))
    update()
  ...
  return (
    <span className="timer-live" style={{ color: 'var(--s-progreso)', fontSize: 12 }}>
      ⏱ {h}h {m.toString().padStart(2, '0')}m corriendo
    </span>
  )
}
export function StatusLegend() {
  const items = [
    { c: 'var(--s-pendiente)', l: 'Pendiente' },
    { c: 'var(--s-progreso)', l: 'En progreso' },
    { c: 'var(--s-pausa)', l: 'En pausa' },
    { c: 'var(--s-revision)', l: 'En revisión' },
    { c: 'var(--s-completada)', l: 'Completada' },
    { c: 'var(--s-vencida)', l: 'Vencida' },
  ]
  return (
    <div className="status-legend">
      {items.map(({ c, l }) => (
        <span key={l} className="status-dot">
          <span
            style={{
              width: 8,
              height: 8,
